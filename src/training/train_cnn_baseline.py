...
from src.data.dataset import EmotionLabelEncoder, compute_class_weights, load_project_metadata
from src.data.spectrogram_dataset import RavdessSpectrogramDataset, SpectrogramConfig, spectrogram_collate_fn
from src.models.cnn_baseline import CnnEmotionClassifier
from src.training.metrics import classification_report_frame, summarize_classification
from src.training.train_wav2vec import get_best_available_device, resolve_project_path
...
def build_spectrogram_config(config: dict[str, Any]) -> SpectrogramConfig:
    return SpectrogramConfig(
        ...
        win_length=int(config.get("win_length", config.get("n_fft", 400))),
        fmin=float(config.get("fmin", 0.0)),
        fmax=float(config["fmax"]) if config.get("fmax") is not None else None,
        target_frames=int(config.get("target_frames", 600)),
    )


def build_dataloaders(config: dict[str, Any]) -> tuple[dict[str, DataLoader], EmotionLabelEncoder]:
    label_encoder = EmotionLabelEncoder()
    metadata_path = resolve_project_path(config["metadata_path"])
    spectrogram_config = build_spectrogram_config(config)
    ...
    train_dataset = RavdessSpectrogramDataset(train_df, label_encoder=label_encoder, spectrogram_config=spectrogram_config)
    val_dataset = RavdessSpectrogramDataset(val_df, label_encoder=label_encoder, spectrogram_config=spectrogram_config)
    test_dataset = RavdessSpectrogramDataset(test_df, label_encoder=label_encoder, spectrogram_config=spectrogram_config)
...
def build_model(config: dict[str, Any], label_encoder: EmotionLabelEncoder) -> CnnEmotionClassifier:
    return CnnEmotionClassifier(
...
def evaluate_model(
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="eval", leave=False):
            ...
            outputs = model(features=features, labels=labels, class_weights=class_weights)
...
def run_training(config: dict[str, Any], dry_run: bool = False) -> None:
    ...
    dataloaders, label_encoder = build_dataloaders(config)
    ...
    if dry_run:
        ...
        print(f"feature_shape={tuple(first_batch['features'].shape)}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a CNN log-Mel baseline on speaker-independent RAVDESS.")
    parser.add_argument(
        "--config",
        type=Path,
